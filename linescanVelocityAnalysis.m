...
if nargin<2
    ...
    prefs.method = 'mean'; 
    %outliers, 0 - it wont remove outliers
...
for a = 1:size(find_tif_file,2)
    ...
    if prefs.pxFlag == 0
        disp('calling magic func to calc pixel size...');
        disp('as setting pxsz in sciscan dint do owt...');
        pref.lsNm='*_RBCV.csv';
        [pxsz, linePxs]= pixel4ls(expDir, width, pref); %this is in microns
    else
        pxsz = str2num(ini_file.x_.x0x2epixel0x2esz) * 1000000; 
        disp(['taking pixel size from ini file: ', num2str(pxsz)]);
    end
    ...
    for k = 1:num_images %loop frames
        ...
        %load other channels
        calc_ch(:,:,k) = imread(find_calc{1,a},k)';
        loco_ch(:,:,k) = imread(find_loco{1,a},k)';
        stim_ch(:,:,k) = imread(find_stim{1,a},k)';
    ...
    find_ww = findFolders(fname, 'wrongWay.txt');
    if size(find_ww,2)>=1
    ...
    save(matfile,'velocity','lineangle','calcium','movement',...
        ...
        'timepts','rawLine','lps','spw','rawLine','prefs2output','-v7.3');
