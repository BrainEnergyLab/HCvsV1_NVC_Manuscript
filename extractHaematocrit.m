...
%% PREFS %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%(as per Drew paper), must be divisble by 4
prefs.windowSz = 40;

% prefs for remove spikes in trace:
...
%prefs for smoothing trace:
prefs.Thresh = 0; %keep at zero - doesnt work well!
...
for a = 22:size(find_tif_file,2) %loop all tif files found
   ...
   stim_ch = cell2mat(findFolders(expDir, '*ch_4.tif'));
    ...
    if height ~= height_ttt || num_images ~= num_images_ttt 
        ...
        save([expDir,filesep,'contData_ls_Hct'],'time','hct',...
            'hct_smooth', 'binaryLine', 'locoLine', 'locomotion', 'stim');
